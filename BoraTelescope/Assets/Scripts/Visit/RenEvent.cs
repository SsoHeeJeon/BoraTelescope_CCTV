...
public class RenEvent : MonoBehaviour
{
    ...
    public void OnclickBack()
    {
        if(Input.touchCount <= 1 && CurrentRen == null)
        {
            if (Wholelist.Count >= 1)
            {
                ...
                else if (Wholelist[Wholelist.Count - 1] == "Back")
                {
                    ...
                    for (int j = 0; j < LRenlist.Count; j++)
                    {
                        try
                        {
                            if (LRenlist[0].gameObject.activeSelf)
                            {
                                if (LRenlist[j].material.color == BackGround.GetComponent<MeshRenderer>().material.color)
                                {
                                    LRenlist[j].material = Matlist[Matlist.Count - 1];
                                }
                                if (Prefab.GetComponent<LineRenderer>().startWidth == 3)
                                {
                                    Prefab.GetComponent<LineRenderer>().material = Matlist[Matlist.Count - 1];
                                }
                            }
                        }
                        catch
                    ...
                    if (Prefab.GetComponent<LineRenderer>().startWidth == 3)
                    {
                        try
                        {
                            if (!Matlist[Matlist.Count - 1].name.Contains("BackGround"))
                            {
                                Prefab.GetComponent<LineRenderer>().material = Matlist[Matlist.Count - 1];
                            }
                            if (Prefab.GetComponent<LineRenderer>().startWidth == 3)
                    ...
                    BackGround.GetComponent<MeshRenderer>().material = Matlist[Matlist.Count - 1];
                    Matlist.RemoveAt(Matlist.Count - 1);
                }
                else if(Wholelist[Wholelist.Count - 1] == "BackChange")
                {
                    for (int j = 0; j < LRenlist.Count; j++)
                    {
                        try
                        {
                            if (LRenlist[j].gameObject.activeSelf)
                            {
                                if (LRenlist[j].material.color == BackGround.GetComponent<MeshRenderer>().material.color)
                                {
                                    LRenlist[j].material = Matlist[Matlist.Count - 1];
                    ...
                    if (backstate == BackState.Change || backstate == BackState.BackGround)
                    {
                        for (int j = 0; j < LRenlist.Count; j++)
                        {
                            LRenlist[j].gameObject.SetActive(true);
                        }

                        for (int k = 0; k < Sti.Stilist.Count; k++)
                        {
                            Sti.Stilist[k].gameObject.SetActive(true);
                        }
                        backstate = BackState.Color;
                    }
                }
                else
                {
    ...
    {
        Matlist.Add(BackGround.GetComponent<MeshRenderer>().material);
        BeforBackGroundobj = obj;
        ...
        switch (obj.name)
        {
            ...
            case "BackBlack":
                ...
                break;
            case "BackGround1":
                if(backstate == BackState.Color)
                {
                    Wholelist.Add("BackChange");
                    backstate = BackState.Change;
                }
                else
                {
                    Wholelist.Add("Back");
                    backstate = BackState.BackGround;
                }
                MenuReset.SetActive(true);
                BackTle.GetComponent<RectTransform>().sizeDelta = new Vector2(197, 172);
                Back.SetActive(true);
                for (int i = 0; i < BackGroundMat.Length; i++)
                {
                    if (BackGroundMat[i].name.Contains("BackGround1"))
                    {
                        ...
                        //Sti.Sticheck = true;
                        //Wholelist.Clear();
                        if(!(backstate == BackState.BackGround || backstate == BackState.None))
                        {
                            for (int j = 0; j < LRenlist.Count; j++)
                            {
                                LRenlist[j].gameObject.SetActive(false);
                            }

                            for (int k = 0; k < Sti.Stilist.Count; k++)
                            {
                                Sti.Stilist[k].gameObject.SetActive(false);
                            }
                        }
                        BackGround.GetComponent<MeshRenderer>().material = BackGroundMat[i];
                        break;
